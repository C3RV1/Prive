...
class MainClass(PriveAPI_mC.MainClassBase):
    ...
    def register(self):
        ...
        while True:
            userName = raw_input("User Name: ")
            password = raw_input("Password: ")
            createUsrRes = self.priveConnection.createUser(userName, password)
            if createUsrRes == PriveAPI_eC.createUser.userAlreadyExists:
                print "User Already Exists"
            elif createUsrRes == PriveAPI_eC.createUser.invalidNameCharacters:
                print "Name Contains Invalid Characters -> /\\:\"?*<>|."
    ...
    def login(self):
        ...
        while True:
            userName = raw_input("User Name: ")
            password = raw_input("Password: ")
            loginResult = self.priveConnection.login(userName, password)
            if loginResult[0] == PriveAPI_eC.login.userNotFound:
                print "User Not Found"
            elif loginResult[0] == PriveAPI_eC.login.incorrectPassword:
                print "Incorrect Password"
    ...
    def menu(self):
        # type: () -> None
        while True:
            if self.priveConnection.loggedIn is True:
                print "Logged in as {0}".format(self.priveConnection.loggedInUser)
            print "1. Register"
            print "2. Login"
            if self.priveConnection.loggedIn is True:
                print "3. Delete User"
            ...
            option = input("Option: ")
            if option == 1:
                register(self.priveConnection)
            elif option == 2:
                login(self.priveConnection)
            elif option == 3:
                if self.priveConnection.loggedIn is True:
                    are_you_sure = raw_input("Are you sure? [y/N]")
                    if are_you_sure == "y" or are_you_sure == "Y":
                        opResult = self.priveConnection.deleteUser()
                        if opResult == PriveAPI_eC.deleteUser.successful:
                            print "User deleted successfully"
                        else:
                            print "Error deleting user"
                    ...
                    break
            elif option == 4 and self.priveConnection.loggedIn is True:
                self.priveConnection.logout()
            elif option == 5 and self.priveConnection.loggedIn is True:
                print "Updating Keys..."
                result = self.priveConnection.updateKeys()
                if result == PriveAPI_eC.updateKeys.successful:
                    print "Keys Updated Sucessfully"
                else:
                    print "Error updating keys"
                    print "Error Code: {0}".format(result)
            elif option == 6 and self.priveConnection.loggedIn is True:
                break
            else:
                print "Option not accepted"
